classdef Utils < handle
    ...
    methods (Static = true)
        function dataout = add_noise(datain,snr,Fs,BW,SF)
            ...
            amp_sig = mean(abs(datain)); % mean amplitude 
            amp_noise = amp_sig/10^(snr/20);
            dlen = length(datain);
            dataout  = datain + (amp_noise/sqrt(2) * randn([1 dlen]) + 1i*amp_noise/sqrt(2) * randn([1 dlen]));
        end
        % amp_noise is computed based on the desired SNR. It's set to a fraction of the signal amplitude.
        % Gaussian noise with the calculated amplitude is generated using the randn function.
        % The noise is added to the input data element-wise, with the real and imaginary parts each being added separately.
        % The noise is scaled by 1/sqrt(2), ensure that the total power of the added noise is consistent with the desired SNR.
        ...
        function symb = gen_symbol(code_word,down,Fs,BW,SF)
            ...
            % 1/Fs: the time duration between two consecutive samples.
            % 2^SF / BW: duration of time required for one full cycle of the chirp signal
            ...
            f0 = -BW/2; % start freq
            f1 = BW/2;  % end freq
            chirpI = chirp(T, f0, 2^SF/BW, f1, 'linear', 90); % time, freq0, time_f1, freq1, method_feq_sweep,phase_offset
            chirpQ = chirp(T, f0, 2^SF/BW, f1, 'linear', 0);
            baseline = complex(chirpI, chirpQ); % Combine I and Q components into a complex baseline signal
            if nargin >= 2 && down
                baseline = conj(baseline); % conjugate of the baseline
            end
            baseline = repmat(baseline,1,2); % Repeat the baseline signal twice
            %             baseline =
            %             [baseline,
            %             baseline*exp(1i*(0))];
            clear chirpI chirpQ
            ...
            % Shift for
            % encoding
            % shifting the baseline signal for encoding purposes and possibly adjusting the sampling rate.
